...
df_pt = pd.read_excel(DATA_DIR / "phuong_tien.xlsx")         # id_phuong_tien, ten_phuong_tien
df_hv = pd.read_excel(DATA_DIR / "hanh_vi.xlsx")             # id_hanh_vi, ten_hanh_vi, loai_hanh_vi, mo_ta
df_dk = pd.read_excel(DATA_DIR / "dieu_kien.xlsx")           # id_dieu_kien, mo_ta
df_vb = pd.read_excel(DATA_DIR / "van_ban_phap_luat.xlsx")   # id_van_ban, ten_van_ban, so_hieu, loai, nam, hieu_luc_tu, hieu_luc_den, tinh_trang
...
def filter_laws(vehicle_kw=None, action_kw=None, condition_kw=None, year=None):
    ...
    # Gọi infer_penalties từ engine, None sẽ bỏ qua filter
...
def display_law_readable(law):
    ...
    htbs = law.get("hinh_thuc_bo_sung")
    htbs_str = htbs if htbs else "Không có"
    ...
    print("===============================================")
    print(f"Luật áp dụng: {law['id_luat']} - {hv_id2name.get(law['hanh_vi'], law['hanh_vi'])} cho {pt_id2name.get(law['phuong_tien'], law['phuong_tien'])}")
    if law.get('dieu_kien'):
        print(f"Điều kiện áp dụng: {dk_id2name.get(law['dieu_kien'], law['dieu_kien'])}")
    print(f"Mức phạt tiền: {min_fine} – {max_fine}")
    print(f"Hình thức xử phạt bổ sung: {htbs_str}")
    print(f"Văn bản pháp luật: {vb_info.get('ten_van_ban', law.get('van_ban'))} ({vb_info.get('so_hieu', '')}, {vb_info.get('loai', '')}, Năm {vb_info.get('nam', '')})")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(f"[DEBUG] Vehicle keyword: {vehicle_kw}, Action keyword: {action_kw}, Condition keyword: {condition_kw}, Year: {year}")
