...
def resolve_alias(text, alias_map):
    ...
    # tách text thành từ rời rạc để match linh hoạt
    words = text.split()
    for phrase, _id in alias_map.items():
        norm_phrase = normalize_text(phrase)
        phrase_words = norm_phrase.split()
        if all(word in words for word in phrase_words):
            matches.append((len(phrase_words), _id))

    if not matches:
        # fallback match theo substring truyền thống
        for phrase, _id in alias_map.items():
            if phrase in text:
                matches.append((len(phrase.split()), _id))
